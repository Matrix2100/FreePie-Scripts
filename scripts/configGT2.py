if starting:    
    ...
    # ==================================
    using_steering_acelerator = True
    steering_aceleration = 3 #1 to 
    sa_a = 1 # changes the curve inclination (1:/, 3:U)
    sa_b = 2 # changes how faster the curve will grow up to max value
    ...
    def steering_acelerator(steering_position):
      x = abs(steering_position/sa_c)
      if x <= 1:
        return float(((x**sa_a)/sa_b)+1)**steering_aceleration
      elif x > 1:
      	return float(((x**sa_a)/sa_b)+1)**steering_aceleration
    ...
    sr_b = 17 # increase the top point (this function is like a mountain /\)
    sr_c = 8000 # That's when the return rate will stop changing (6000 its like 30% of the steering wheel)
    sr_d = 4 # return rate multiplier
    sr_e = 0.3 # reduces the minimum value for the return rate (you'll need to compensate that changing the sr_b)
